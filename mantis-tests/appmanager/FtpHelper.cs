...
namespace mantis_tests.appmanager
{
    public class FtpHelper : HelperBase
    {
        ...
        public void BackupFile(string path) // существующий конф.файл "припрятать", чтобы потом восстановаить 
        {
        ...
        public void RestoreBackupFile(string path)// восстанавлиает из резервной копии
        {
            String backupPath = path + ".bak";
            if (!client.FileExists(backupPath))
            {
        ...
        public void Upload(string path, Stream localFile)
        {
            ...
            using (Stream ftpStream = client.OpenWrite(path))
            {
                ...
                int count = localFile.Read(buffer, 0, buffer.Length);//read соклько байтов реально прочитано 
                while (count > 0)//читаем пока читается 
                {
